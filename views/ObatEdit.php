...
<form name="fobatedit" id="fobatedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
...
<?php if ($Page->id_obat->Visible) { // id_obat ?>
    <div id="r_id_obat"<?= $Page->id_obat->rowAttributes() ?>>
        <label id="elh_obat_id_obat" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_obat->caption() ?><?= $Page->id_obat->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id_obat->cellAttributes() ?>>
<span id="el_obat_id_obat">
<span<?= $Page->id_obat->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_obat->getDisplayValue($Page->id_obat->getEditValue()))) ?>"></span>
<input type="hidden" data-table="obat" data-field="x_id_obat" data-hidden="1" name="x_id_obat" id="x_id_obat" value="<?= HtmlEncode($Page->id_obat->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script<?= Nonce() ?>>
loadjs.ready(['fobatedit', 'jqueryinputmask'], function() {
	options = {
		'alias': 'numeric',
		'autoUnmask': true,
		'jitMasking': false,
	...
	ew.createjQueryInputMask("fobatedit", "x_harga", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
loadjs.ready(['fobatedit', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fobatedit", "x_stok", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<script<?= Nonce() ?>>
loadjs.ready(["fobatedit", "datetimepicker"], function () {
...
loadjs.ready(['fobatedit', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fobatedit", "x_expired_date", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fobatedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fobatedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fobatedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
