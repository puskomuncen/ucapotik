...
<form name="fdokteredit" id="fdokteredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
...
<?php if ($Page->id_dokter->Visible) { // id_dokter ?>
    <div id="r_id_dokter"<?= $Page->id_dokter->rowAttributes() ?>>
        <label id="elh_dokter_id_dokter" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_dokter->caption() ?><?= $Page->id_dokter->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_dokter->getDisplayValue($Page->id_dokter->getEditValue()))) ?>"></span>
<input type="hidden" data-table="dokter" data-field="x_id_dokter" data-hidden="1" name="x_id_dokter" id="x_id_dokter" value="<?= HtmlEncode($Page->id_dokter->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script<?= Nonce() ?>>
loadjs.ready(['fdokteredit', 'jqueryinputmask'], function() {
	options = {
		'alias': 'numeric',
		'autoUnmask': true,
		'jitMasking': false,
	...
	ew.createjQueryInputMask("fdokteredit", "x_tarif_konsultasi", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdokteredit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdokteredit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fdokteredit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
