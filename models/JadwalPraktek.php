...
/**
 * Table class for jadwal_praktek
 */
class JadwalPraktek extends DbTable implements LookupTableInterface
{
    ...
    public DbField $hari;
    public DbField $jam_mulai;
    public DbField $jam_selesai;
    public DbField $kuota;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "jadwal_praktek"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->id_jadwal->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        // hari
        $this->hari = new DbField(
            $this, // Table
            'x_hari', // Variable name
            'hari', // Name
            '`hari`', // Expression
            '`hari`', // Basic search expression
            200, // Type
            6, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`hari`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->hari->OptionCount = 7;
        $this->hari->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['hari'] = &$this->hari;

        // jam_mulai
        $this->jam_mulai = new DbField(
            $this, // Table
            'x_jam_mulai', // Variable name
            'jam_mulai', // Name
            '`jam_mulai`', // Expression
            CastDateFieldForLike("`jam_mulai`", 4, "DB"), // Basic search expression
            134, // Type
            10, // Size
            4, // Date/Time format
            false, // Is upload field
            '`jam_mulai`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->jam_mulai->InputTextType = "text";
        $this->jam_mulai->Raw = true;
        $this->jam_mulai->Nullable = false; // NOT NULL field
        $this->jam_mulai->Required = true; // Required field
        $this->jam_mulai->DefaultErrorMessage = str_replace("%s", DateFormat(4), $this->language->phrase("IncorrectTime"));
        $this->jam_mulai->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['jam_mulai'] = &$this->jam_mulai;

        // jam_selesai
        $this->jam_selesai = new DbField(
            $this, // Table
            'x_jam_selesai', // Variable name
            'jam_selesai', // Name
            '`jam_selesai`', // Expression
            CastDateFieldForLike("`jam_selesai`", 4, "DB"), // Basic search expression
            134, // Type
            10, // Size
            4, // Date/Time format
            false, // Is upload field
            '`jam_selesai`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->jam_selesai->InputTextType = "text";
        $this->jam_selesai->Raw = true;
        $this->jam_selesai->Nullable = false; // NOT NULL field
        $this->jam_selesai->Required = true; // Required field
        $this->jam_selesai->DefaultErrorMessage = str_replace("%s", DateFormat(4), $this->language->phrase("IncorrectTime"));
        $this->jam_selesai->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['jam_selesai'] = &$this->jam_selesai;

        // kuota
        $this->kuota = new DbField(
            $this, // Table
            'x_kuota', // Variable name
            'kuota', // Name
            '`kuota`', // Expression
            '`kuota`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`kuota`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->kuota->addMethod("getDefault", fn() => 20);
        $this->kuota->InputTextType = "text";
        $this->kuota->Raw = true;
        $this->kuota->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->kuota->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['kuota'] = &$this->kuota;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "jadwal_praktek";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            ...
            $row['id_jadwal'] = $this->id_jadwal->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_jadwal']) && !IsEmpty($this->id_jadwal->CurrentValue)) {
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_jadwal', $row)) {
                AddFilter($where, QuotedName('id_jadwal', $this->Dbid) . '=' . QuotedValue($row['id_jadwal'], $this->id_jadwal->DataType, $this->Dbid));
            }
        }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            ...
            $val = !IsEmpty($this->id_jadwal->OldValue) && !$current ? $this->id_jadwal->OldValue : $this->id_jadwal->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_jadwal@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("jadwalprakteklist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "jadwalpraktekview" => $this->language->phrase("View"),
            "jadwalpraktekedit" => $this->language->phrase("Edit"),
            "jadwalpraktekadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "JadwalPraktekView",
            Config("API_ADD_ACTION") => "JadwalPraktekAdd",
            Config("API_EDIT_ACTION") => "JadwalPraktekEdit",
            Config("API_DELETE_ACTION") => "JadwalPraktekDelete",
            Config("API_LIST_ACTION") => "JadwalPraktekList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jadwalpraktekview", $parm);
        } else {
            $url = $this->keyUrl("jadwalpraktekview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "jadwalpraktekadd?" . $parm;
        } else {
            $url = "jadwalpraktekadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jadwalpraktekedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jadwalprakteklist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jadwalpraktekadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jadwalprakteklist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jadwalpraktekdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_jadwal\":" . VarToJson($this->id_jadwal->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl(string $url, string $parm = ""): string
    {
        if ($this->id_jadwal->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_jadwal->CurrentValue);
        } else {
    ...
    public function renderLookupForView(string $name, mixed $value): mixed
    {
        ...
        if ($name == "hari") {
            $clone = $this->hari->getClone()->setViewValue($value);
