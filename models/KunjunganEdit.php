 ...
 * Page class
 */
class KunjunganEdit extends Kunjungan
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "KunjunganEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "kunjunganedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_kunjungan") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_kunjungan->setQueryStringValue($keyValue);
                $this->id_kunjungan->setOldValue($this->id_kunjungan->QueryStringValue);
            } elseif (Post("id_kunjungan") !== null) {
                $this->id_kunjungan->setFormValue(Post("id_kunjungan"));
                $this->id_kunjungan->setOldValue($this->id_kunjungan->FormValue);
            } else {
...
// Get upload files
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // id_pasien
        $this->id_pasien->setDbValueDef($newRow, $this->id_pasien->CurrentValue, $this->id_pasien->ReadOnly);

        // id_dokter
        $this->id_dokter->setDbValueDef($newRow, $this->id_dokter->CurrentValue, $this->id_dokter->ReadOnly);

        // id_jadwal
        $this->id_jadwal->setDbValueDef($newRow, $this->id_jadwal->CurrentValue, $this->id_jadwal->ReadOnly);
        ...
        // resep
        $this->resep->setDbValueDef($newRow, $this->resep->CurrentValue, $this->resep->ReadOnly);
    ...
    {
        if (isset($row['id_pasien'])) { // id_pasien
            $this->id_pasien->CurrentValue = $row['id_pasien'];
        }
        if (isset($row['id_dokter'])) { // id_dokter
            $this->id_dokter->CurrentValue = $row['id_dokter'];
        }
        if (isset($row['id_jadwal'])) { // id_jadwal
            $this->id_jadwal->CurrentValue = $row['id_jadwal'];
        }
        ...
        if (isset($row['resep'])) { // resep
            $this->resep->CurrentValue = $row['resep'];
        }
