 ...
 */
class Pasien extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "pasien"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->id_pasien->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        // no_telp
        $this->no_telp = new DbField(
            $this, // Table
            'x_no_telp', // Variable name
            'no_telp', // Name
            '`no_telp`', // Expression
            '`no_telp`', // Basic search expression
            200, // Type
            15, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`no_telp`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->no_telp->InputTextType = "text";
        $this->no_telp->Nullable = false; // NOT NULL field
        $this->no_telp->Required = true; // Required field
        $this->no_telp->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        ...
        // alergi
        $this->alergi = new DbField(
            $this, // Table
            'x_alergi', // Variable name
            'alergi', // Name
            '`alergi`', // Expression
            '`alergi`', // Basic search expression
            200, // Type
            65535, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`alergi`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->alergi->InputTextType = "text";
        $this->alergi->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            ...
            $row['id_pasien'] = $this->id_pasien->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_pasien']) && !IsEmpty($this->id_pasien->CurrentValue)) {
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_pasien', $row)) {
                AddFilter($where, QuotedName('id_pasien', $this->Dbid) . '=' . QuotedValue($row['id_pasien'], $this->id_pasien->DataType, $this->Dbid));
            }
        }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            ...
            $val = !IsEmpty($this->id_pasien->OldValue) && !$current ? $this->id_pasien->OldValue : $this->id_pasien->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_pasien@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("pasienlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "pasienview" => $this->language->phrase("View"),
            "pasienedit" => $this->language->phrase("Edit"),
            "pasienadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PasienView",
            Config("API_ADD_ACTION") => "PasienAdd",
            Config("API_EDIT_ACTION") => "PasienEdit",
            Config("API_DELETE_ACTION") => "PasienDelete",
            Config("API_LIST_ACTION") => "PasienList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("pasienview", $parm);
        } else {
            $url = $this->keyUrl("pasienview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pasienedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pasienlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pasienadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pasienlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("pasiendelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_pasien\":" . VarToJson($this->id_pasien->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl(string $url, string $parm = ""): string
    {
        if ($this->id_pasien->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_pasien->CurrentValue);
        } else {
