...
class DokterList extends Dokter
{
    ...
    // Page object name
    public string $PageObjName = "DokterList";
    ...
    public string $FormName = "fdokterlist";

    // CSS class/style
    public string $CurrentPageName = "dokterlist";
    ...
    public function setVisibility(): void
    {
        ...
        $this->spesialisasi->setVisibility();
        $this->no_lisensi->setVisibility();
        $this->tarif_konsultasi->setVisibility();
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Table object (dokter)
        if (!isset($GLOBALS["dokter"]) || $GLOBALS["dokter"]::class == PROJECT_NAMESPACE . "dokter") {
            $GLOBALS["dokter"] = &$this;
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "dokterview"); // If View page, no primary button
                } else { // List page
...
// Is modal
		...
		if (ReadCookie('dokter_searchpanel') == 'notactive' || ReadCookie('dokter_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('dokter_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fdoktersrch");
        }
        ...
        $filterList = Concat($filterList, $this->spesialisasi->AdvancedSearch->toJson(), ","); // Field spesialisasi
        $filterList = Concat($filterList, $this->no_lisensi->AdvancedSearch->toJson(), ","); // Field no_lisensi
        $filterList = Concat($filterList, $this->tarif_konsultasi->AdvancedSearch->toJson(), ","); // Field tarif_konsultasi
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fdoktersrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field spesialisasi
        $this->spesialisasi->AdvancedSearch->SearchValue = $filter["x_spesialisasi"] ?? "";
        $this->spesialisasi->AdvancedSearch->SearchOperator = $filter["z_spesialisasi"] ?? "";
        $this->spesialisasi->AdvancedSearch->SearchCondition = $filter["v_spesialisasi"] ?? "";
        $this->spesialisasi->AdvancedSearch->SearchValue2 = $filter["y_spesialisasi"] ?? "";
        $this->spesialisasi->AdvancedSearch->SearchOperator2 = $filter["w_spesialisasi"] ?? "";
        $this->spesialisasi->AdvancedSearch->save();

        // Field no_lisensi
        $this->no_lisensi->AdvancedSearch->SearchValue = $filter["x_no_lisensi"] ?? "";
        $this->no_lisensi->AdvancedSearch->SearchOperator = $filter["z_no_lisensi"] ?? "";
        $this->no_lisensi->AdvancedSearch->SearchCondition = $filter["v_no_lisensi"] ?? "";
        $this->no_lisensi->AdvancedSearch->SearchValue2 = $filter["y_no_lisensi"] ?? "";
        $this->no_lisensi->AdvancedSearch->SearchOperator2 = $filter["w_no_lisensi"] ?? "";
        $this->no_lisensi->AdvancedSearch->save();

        // Field tarif_konsultasi
        $this->tarif_konsultasi->AdvancedSearch->SearchValue = $filter["x_tarif_konsultasi"] ?? "";
        $this->tarif_konsultasi->AdvancedSearch->SearchOperator = $filter["z_tarif_konsultasi"] ?? "";
        $this->tarif_konsultasi->AdvancedSearch->SearchCondition = $filter["v_tarif_konsultasi"] ?? "";
        $this->tarif_konsultasi->AdvancedSearch->SearchValue2 = $filter["y_tarif_konsultasi"] ?? "";
        $this->tarif_konsultasi->AdvancedSearch->SearchOperator2 = $filter["w_tarif_konsultasi"] ?? "";
        $this->tarif_konsultasi->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        $searchFlds[] = &$this->spesialisasi;
        $searchFlds[] = &$this->no_lisensi;
        $searchFlds[] = &$this->no_telp;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->spesialisasi); // spesialisasi
            $this->updateSort($this->no_lisensi); // no_lisensi
            $this->updateSort($this->tarif_konsultasi); // tarif_konsultasi
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->nama->setSort("");
                $this->spesialisasi->setSort("");
                $this->no_lisensi->setSort("");
                $this->tarif_konsultasi->setSort("");
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"dokter\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"dokter\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"dokter\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdokterlist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdokterlist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_dokter->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"dokter\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "id_dokter");
            $this->createColumnOption($option, "nama");
            $this->createColumnOption($option, "spesialisasi");
            $this->createColumnOption($option, "no_lisensi");
            $this->createColumnOption($option, "tarif_konsultasi");
            $this->createColumnOption($option, "no_telp");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdoktersrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdoktersrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdokterlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_dokter", "data-rowtype" => RowType::ADD]);
...
// Load basic search values
    ...
    public function loadRowValues(array|bool|null $row = null): void
    {
        ...
        $this->spesialisasi->setDbValue($row['spesialisasi']);
        $this->no_lisensi->setDbValue($row['no_lisensi']);
        $this->tarif_konsultasi->setDbValue($row['tarif_konsultasi']);
    ...
    protected function newRow(): array
    {
        ...
        $row['spesialisasi'] = $this->spesialisasi->DefaultValue;
        $row['no_lisensi'] = $this->no_lisensi->DefaultValue;
        $row['tarif_konsultasi'] = $this->tarif_konsultasi->DefaultValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // no_lisensi
            $this->no_lisensi->ViewValue = $this->no_lisensi->CurrentValue;

            // tarif_konsultasi
            $this->tarif_konsultasi->ViewValue = $this->tarif_konsultasi->CurrentValue;
            $this->tarif_konsultasi->ViewValue = FormatNumber($this->tarif_konsultasi->ViewValue, $this->tarif_konsultasi->formatPattern());
            ...
            // no_lisensi
            $this->no_lisensi->HrefValue = "";
            $this->no_lisensi->TooltipValue = "";

            // tarif_konsultasi
            $this->tarif_konsultasi->HrefValue = "";
            $this->tarif_konsultasi->TooltipValue = "";
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('dokter_searchpanel') == 'notactive' || ReadCookie('dokter_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdoktersrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('dokter_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdoktersrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdoktersrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
