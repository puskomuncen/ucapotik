...
/**
 * Table class for obat
 */
class Obat extends DbTable implements LookupTableInterface
{
    ...
    public DbField $nama_obat;
    public DbField $kategori;
    public DbField $harga;
    public DbField $stok;
    public DbField $expired_date;
    public DbField $satuan;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "obat"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->id_obat->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['id_obat'] = &$this->id_obat;

        // nama_obat
        $this->nama_obat = new DbField(
            $this, // Table
            'x_nama_obat', // Variable name
            'nama_obat', // Name
            '`nama_obat`', // Expression
            '`nama_obat`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nama_obat`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nama_obat->InputTextType = "text";
        $this->nama_obat->Nullable = false; // NOT NULL field
        $this->nama_obat->Required = true; // Required field
        $this->nama_obat->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['nama_obat'] = &$this->nama_obat;

        // kategori
        $this->kategori = new DbField(
            $this, // Table
            'x_kategori', // Variable name
            'kategori', // Name
            '`kategori`', // Expression
            '`kategori`', // Basic search expression
            200, // Type
            50, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`kategori`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->kategori->InputTextType = "text";
        $this->kategori->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->harga->DefaultErrorMessage = $this->language->phrase("IncorrectFloat");
        ...
        $this->stok->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->stok->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['stok'] = &$this->stok;

        // expired_date
        $this->expired_date = new DbField(
            $this, // Table
            'x_expired_date', // Variable name
            'expired_date', // Name
            '`expired_date`', // Expression
            CastDateFieldForLike("`expired_date`", 0, "DB"), // Basic search expression
            133, // Type
            10, // Size
            0, // Date/Time format
            false, // Is upload field
            '`expired_date`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->expired_date->Raw = true;
        $this->expired_date->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $this->language->phrase("IncorrectDate"));
        $this->expired_date->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['expired_date'] = &$this->expired_date;

        // satuan
        $this->satuan = new DbField(
            $this, // Table
            'x_satuan', // Variable name
            'satuan', // Name
            '`satuan`', // Expression
            '`satuan`', // Basic search expression
            200, // Type
            20, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`satuan`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->satuan->addMethod("getDefault", fn() => "tablet");
        $this->satuan->InputTextType = "text";
        $this->satuan->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "obat";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            ...
            $row['id_obat'] = $this->id_obat->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_obat']) && !IsEmpty($this->id_obat->CurrentValue)) {
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_obat', $row)) {
                AddFilter($where, QuotedName('id_obat', $this->Dbid) . '=' . QuotedValue($row['id_obat'], $this->id_obat->DataType, $this->Dbid));
            }
        }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            ...
            $val = !IsEmpty($this->id_obat->OldValue) && !$current ? $this->id_obat->OldValue : $this->id_obat->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_obat@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("obatlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "obatview" => $this->language->phrase("View"),
            "obatedit" => $this->language->phrase("Edit"),
            "obatadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ObatView",
            Config("API_ADD_ACTION") => "ObatAdd",
            Config("API_EDIT_ACTION") => "ObatEdit",
            Config("API_DELETE_ACTION") => "ObatDelete",
            Config("API_LIST_ACTION") => "ObatList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("obatview", $parm);
        } else {
            $url = $this->keyUrl("obatview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("obatlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("obatlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("obatdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_obat\":" . VarToJson($this->id_obat->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl(string $url, string $parm = ""): string
    {
        if ($this->id_obat->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_obat->CurrentValue);
        } else {
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->nama_obat);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->nama_obat);
    ...
    public function renderLookupForView(string $name, mixed $value): mixed
    {
        ...
        if ($name == "nama_obat") {
            $clone = $this->nama_obat->getClone()->setViewValue($value);
