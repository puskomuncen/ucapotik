...
function LoadApplicationSettings() {
	$conn = Conn();
	$_SESSION["apotik2025baru_views"] = 1; // reset the global counter
	// Parent array of all items, initialized if not already...
	if (!isset($_SESSION["apotik2025baru_appset"])) {
		$_SESSION["apotik2025baru_appset"] = array();
	}
	...
	if ($stmt->rowCount() > 0) {
		while ($row = $stmt->fetchAssociative()) {
			...
			for ($i=0; $i<count($x); $i++) {
				if (is_string($x[$i])) {
					$sfieldname = $x[$i];
					$_SESSION["apotik2025baru_appset"][0][$sfieldname] = $row[$x[$i]];
				}
			}
		}
		if (!isset($_SESSION["apotik2025baru_errordb"]))
			$_SESSION["apotik2025baru_errordb"] = "";
	} else {
		if (!isset($_SESSION["apotik2025baru_errordb"]))
			$_SESSION["apotik2025baru_errordb"] = Config("MS_SETTINGS_TABLE");
	}
}
...
function My_Global_Check() {
	...
	if (!isset($_SESSION["apotik2025baru_Root_URL"])) { 
		$_SESSION["apotik2025baru_Root_URL"] = AppRootURL();
	}
	if (IsLoggedIn()) {
        if (!IsAdmin()) {
	...
	if (!isset($_SESSION["apotik2025baru_views"])) { 
		$_SESSION["apotik2025baru_views"] = 0;
	}
	$_SESSION["apotik2025baru_views"] = $_SESSION["apotik2025baru_views"]+1;
	if (!isset($_SESSION["apotik2025baru_appset"])) {
		LoadApplicationSettings();
	}
	if (@$_SESSION["apotik2025baru_appset"][0]["Show_Announcement"]=="Y") {
		Config("MS_SHOW_ANNOUNCEMENT", TRUE);
	} else {
		Config("MS_SHOW_ANNOUNCEMENT", FALSE);      
	}
	if (@$_SESSION["apotik2025baru_appset"][0]["Use_Announcement_Table"]=="Y") {
		Config("MS_SEPARATED_ANNOUNCEMENT", TRUE);
	} else {
		Config("MS_SEPARATED_ANNOUNCEMENT", FALSE);      
	}
	if (@$_SESSION["apotik2025baru_appset"][0]["Maintenance_Mode"]=="Y") {
		Config("MS_MAINTENANCE_MODE", TRUE);        
	} else {
		Config("MS_MAINTENANCE_MODE", FALSE);
	}
	if (@$_SESSION["apotik2025baru_appset"][0]["Maintenance_Finish_DateTime"]!="") {
		Config("MS_MAINTENANCE_END_DATETIME", $_SESSION["apotik2025baru_appset"][0]["Maintenance_Finish_DateTime"]);        
	} else {
		Config("MS_MAINTENANCE_END_DATETIME", "");
	}
	if (@$_SESSION["apotik2025baru_appset"][0]["Auto_Normal_After_Maintenance"]=="Y") {
